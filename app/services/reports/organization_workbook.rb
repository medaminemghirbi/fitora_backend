...
module Reports
  ...
  class OrganizationWorkbook
    ...
    def self.call(organization:, period:)
    ...
    attr_reader :organization, :period, :package
    ...
    def build_summary_sheet
      package.workbook.add_worksheet(name: "Résumé") do |sheet|
        sheet.add_row [ "Rapport Fitora — #{organization.name}" ], style: title_style
    ...
    def summary_rows
      [
        ...
        [ "Clients actifs", organization.clients.active.count ],
        [ "Clients inactifs", organization.clients.where(active: false).count ],
        [ "Abonnements actifs", organization.memberships.currently_active.count ],
        [ "Abonnements expirés", organization.memberships.expired.count ],
        [ "Membres d'équipe", organization.staff_members.count ]
      ]
    end

    def paid_payments_in_period
      @paid_payments_in_period ||= organization.payments.paid.where(paid_at: period.range)
    end
    ...
    def build_clients_sheet
      package.workbook.add_worksheet(name: "Clients") do |sheet|
        ...
        organization.clients.order(:first_name, :last_name).find_each do |client|
