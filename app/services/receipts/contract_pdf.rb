...
module Receipts
  # Builds the "receipt/invoice" PDF for one contract — the
  ...
  # on demand (ContractsController#receipt), not stored anywhere.
  class ContractPdf
    BRAND = "4F46E5".freeze
    ...
    def title(pdf)
      ...
      pdf.text "N° #{contract.id.split('-').first.upcase}", size: 9, color: "555555"
      pdf.move_down 12
    end
    ...
    def plan_details(pdf)
      rows = [
        [ "Forfait", plan.name ],
        [ "Période", "#{contract.starts_at.to_date.strftime('%d/%m/%Y')} au #{contract.expires_at&.to_date&.strftime('%d/%m/%Y') || 'Illimitée'}" ],
        [ "Prix du forfait", format_money(plan.price, plan.currency) ],
        [ "Remise", format_money(contract.discount, plan.currency) ],
        [ "Prix final", format_money(contract.final_price, plan.currency) ],
    ...
    def payments_table(pdf)
      ...
      paid_payments = contract.payments.paid.order(:paid_at)
    ...
    def totals(pdf)
      total_paid = contract.payments.paid.sum(:amount)
      balance = [ contract.final_price.to_f - total_paid.to_f, 0 ].max
    ...
    def payment_status_label
      { "unpaid" => "Impayé", "partial" => "Partiellement payé", "paid" => "Payé intégralement" }.fetch(contract.payment_status, contract.payment_status)
    end
