...
RSpec.describe Bookings::SendReminder do
  let(:organization) { create(:organization, name: "Fitora Test Gym") }
  ...
  describe "#call" do
    it "sends an SMS to the client's normalized mobile number" do
      client = create(:client, organization: organization, first_name: "Ines", phone: "+216 20 111 222")
    ...
    it "fills in the 216 country code for a local 8-digit number" do
      client = create(:client, organization: organization, phone: "20 111 222")
      booking = create(:booking, client: client, session: session)
    ...
    it "fails gracefully when the client has no usable phone number" do
      client = create(:client, organization: organization, phone: "n/a")
      booking = create(:booking, client: client, session: session)
    ...
    it "reports a gateway failure instead of raising" do
      client = create(:client, organization: organization, phone: "+216 20 111 222")
      booking = create(:booking, client: client, session: session)
